"""
Here we suggest very simple approach that adding MLP to handle displacement & Rotation of 
3D-GS
...
class ImplicitNetwork(nn.Module):
    def __init__(
            ...
            skip_in=-1,
            weight_norm=True,
            multires=0,                         # used 10 in NeRF, 4 in pgdna.
            pose_cond_layer=[],
            pose_cond_dim=-1,
            pose_embed_dim=-1,
            shape_cond_layer=[],
        ...
        for l in range(0, self.num_layers - 1):
            ...
            elif zero_conv and (l == self.num_layers - 2):
                torch.nn.init.constant_(lin.weight, 0.0)
                torch.nn.init.constant_(lin.bias, 0.0)
    ...
    def forward(self, input, cond, input_feat=None, return_feat=False, val_pad=0, normalize=False):
