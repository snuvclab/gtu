...
if __name__ == "__main__":
    ...
    output_dir = data_dir / 'people_segmentation'
    ...
    with torch.no_grad():
        for fid in tqdm(sorted(list(img_fname_dict.keys())), desc="People Segmentations"):
            ...
            if len(transformed_boxes) > 0:
                ...
                human_mask = masks.detach().cpu().numpy().sum(axis=0) > 0
                human_mask = (human_mask * 255).astype(np.uint8)
            else:
                human_mask = np.zeros((H,W), dtype=np.uint8)

            cv2.imwrite(str(output_dir / f"{fid:06}.png"), human_mask)
